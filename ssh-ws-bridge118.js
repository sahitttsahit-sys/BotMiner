'use strict';
// SSH-over-WebSocket client bridge (Node v22, no npm deps).
// Menghubungkan SSH client (PuTTY/ssh) di 127.0.0.1:<port> ke server ssh-ws
// lewat WebSocket publik (HTTPS/Caddy L7), yang lalu di-pipe ke sshd:2222.
//
// Mode listener (default):  node ssh-ws-bridge.js [URL] [PORT]
//   -> dengar 127.0.0.1:PORT (default 2222), PuTTY connect ke sana.
// Mode stdio (ProxyCommand): node ssh-ws-bridge.js --stdio [URL]
//   -> pump stdin/stdout ssh <-> WebSocket.

const net = require('net');
const DEFAULT_URL = 'https://a84f3b0499e9badc6.sg.agentos-app.run/ssh';
...
function stdioMode(url){
  ...
  // WAJIB: jangan resume() stdin sebelum ws 'open', else data hilang -> "Bad packet length".
