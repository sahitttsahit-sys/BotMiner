'use strict';
// SSH-over-WebSocket client bridge (Node v22, tanpa dependency npm)
// Dua mode:
//   listener (default): node ssh-ws-bridge.js [url] [port]   -> dengar 127.0.0.1:port
//   ProxyCommand:        node ssh-ws-bridge.js --stdio [url]
const net = require('net');

const DEFAULT_URL = 'wss://a24773dcb96b0b61d.sg.agentos-app.run/ssh';
...
// Normalisasi: https:// -> wss:// , http:// -> ws:// (WebSocket butuh skema ws/wss)
...
if (a[0]==='--stdio') stdioMode(normalizeUrl(a[1]));
  ...
  const url = normalizeUrl(a[0] && !/^\d+$/.test(a[0]) ? a[0] : null);
  // Jika argumen pertama adalah angka -> itu port, pakai DEFAULT_URL
  let port = 2222;
  if (a.length >= 1 && /^\d+$/.test(a[0])) port = Number(a[0]);
  else if (a.length >= 2) port = Number(a[1]);
  listenMode(url, port);
}
