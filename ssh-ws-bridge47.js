...
const DEFAULT_URL = 'https://af226645a857ad6d3.sg2.agentos-app.run/ssh';

// Node v22 punya WebSocket global (tanpa dependency npm). Untuk <22 butuh `npm i ws`.
...
// Mode ProxyCommand (stdin/stdout) — dipakai lewat `ssh -o ProxyCommand=...`
function stdioMode(url){
  const ws = dial(url);
  const pending=[]; let opened=false;
  // PENTING: jangan resume stdin sebelum ws 'open', kalau tidak data hilang -> "Bad packet length".
...
// Mode listener (default) — dengar TCP lokal, teruskan ke WebSocket publik
