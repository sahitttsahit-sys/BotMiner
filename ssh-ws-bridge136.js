...
// Diisi otomatis dari publish.js (link permanen platform, derive dari AGENTOS_RUNTIME_ID via md5)
const DEFAULT_URL = 'https://a98a1e304866d5740.sg.agentos-app.run/ssh';

// Node v22 punya WebSocket global (tanpa dependency npm). Untuk Node <22 install `ws`.
...
// Mode ProxyCommand (stdin/stdout) — untuk dipakai via ssh -o ProxyCommand
// PENTING: buffer stdin sampai ws 'open' agar tidak kehilangan data -> "Bad packet length"
function stdioMode(url){
  const ws = dial(url);
  const pending=[]; let opened=false;
  // JANGAN resume stdin sebelum ws open; tapi kita tetap dengarkan dan antrikan
...
// Mode listener (default) — buka TCP listener 127.0.0.1:port, setiap koneksi dial WS baru
