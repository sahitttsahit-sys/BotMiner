...
// Diisi otomatis dari shareLink permanen (step 4 publish).
const DEFAULT_URL = 'https://af29cbd73d7dc6c7d.sg2.agentos-app.run/ssh';
...
// Normalisasi skema: global WebSocket Node v22 terima https, tapi spec wajibnya ws/wss.
function normalizeUrl(url){
  if (/^https:\/\//.test(url)) return url.replace(/^https:\/\//, 'wss://');
  if (/^http:\/\//.test(url))  return url.replace(/^http:\/\//,  'ws://');
  return url;
}
...
function attach(ws, onOpen, onData, onEnd, onErr){
  if (ws.addEventListener){
    ...
    ws.on('open', onOpen); ws.on('message', d => onData(Buffer.from(d)));
    ws.on('close', () => onEnd()); ws.on('error', () => onErr());
  }
}

function dial(url){
  const w = new WS(normalizeUrl(url));
  try { if (w.binaryType !== undefined) w.binaryType = 'arraybuffer'; } catch {}
  return w;
}

// Mode ProxyCommand (stdin/stdout) — dipanggil: node ssh-ws-bridge.js --stdio <url>
...
// Mode listener (TCP lokal -> WS) — dipanggil: node ssh-ws-bridge.js [url] [port]
