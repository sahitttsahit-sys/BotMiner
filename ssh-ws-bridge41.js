'use strict';
// SSH-over-WebSocket bridge (KLIEN) — Node v22, tanpa dependency npm.
// Memakai WebSocket GLOBAL Node v22 (globalThis.WebSocket).
//
// Dua mode:
//   1) Listener (default): dengar 127.0.0.1:<port> lalu teruskan tiap koneksi TCP
//      ke WebSocket DEFAULT_URL. Cocok untuk PuTTY/Terminal biasa.
//      Contoh: node ssh-ws-bridge.js                    # pakai DEFAULT_URL & port 2222
//              node ssh-ws-bridge.js <url> <port>
//   2) --stdio: ProxyCommand stdin/stdout. Contoh:
//        ProxyCommand node ssh-ws-bridge.js --stdio <url>
//
// CATATAN PENTING (jebakan "Bad packet length"):
//   Pada mode --stdio, jangan panggil process.stdin.resume() SEBELUM ws 'open'.
//   Data stdin yang masuk sebelum ws open di-buffer dulu, baru di-flush saat open.

const net = require('net');

// === ISI DENGAN SHARELINK DARI LANGKAH PUBLISH (akhiri dengan /ssh) ===
const DEFAULT_URL = 'https://a66de1412bcc956f9.sg.agentos-app.run/ssh';
