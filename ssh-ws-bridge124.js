...
// Memakai WebSocket GLOBAL bawaan Node v22 (globalThis.WebSocket).
//
// Dua mode:
//   1) Listener (default): dengar TCP di 127.0.0.1:<port> (default 2222),
//      setiap koneksi TCP di-pipe dua arah ke WebSocket <url>/ssh.
//      Cocok untuk PuTTY / ssh biasa -> connect 127.0.0.1:<port>.
//   2) --stdio <url>: mode ProxyCommand (stdin/stdout <-> WebSocket),
//      dipakai langsung lewat `ssh -o ProxyCommand='node ssh-ws-bridge.js --stdio <url>'`.
//
// Pakai:
//   node ssh-ws-bridge.js [url] [port]            # listener mode
//   node ssh-ws-bridge.js --stdio [url]           # stdio / ProxyCommand mode
//
// DEFAULT_URL = link publik permanen (derive dari AGENTOS_RUNTIME_ID).

const net = require('net');

const DEFAULT_URL = 'https://a6ba690167616da8e.sg2.agentos-app.run/ssh';
...
// --- Mode stdio (ProxyCommand) ---
// PENTING: jangan resume() stdin SEBELUM ws 'open', kalau tidak data awal
// hilang -> "Bad packet length". Buffer stdin sampai ws open.
