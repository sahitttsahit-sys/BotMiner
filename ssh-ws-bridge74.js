...
// Memakai WebSocket GLOBAL bawaan Node v22+. Untuk Node <22, install `ws` (npm i ws).
//
// Dua mode:
//   1) LISTENER (default):
//        node ssh-ws-bridge.js [url] [portLokal]
//        contoh: node ssh-ws-bridge.js https://.../ssh 2222
//        -> dengar 127.0.0.1:2222, PuTTY/ssh connect ke 127.0.0.1:2222
//   2) STDIO (untuk ProxyCommand / `ssh -o ProxyCommand=...`):
//        node ssh-ws-bridge.js --stdio [url]
//        contoh: node ssh-ws-bridge.js --stdio https://.../ssh

const net = require('net');
const DEFAULT_URL = 'https://a297e05b20cd17bb0.sg2.agentos-app.run/ssh';
...
function stdioMode(url) {
  ...
  const pending = [];
  let opened = false;
  // Buffer stdin sampai ws 'open' agar tidak ada "Bad packet length" (data hilang).
