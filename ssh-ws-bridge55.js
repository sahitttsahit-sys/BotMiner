...
// URL WebSocket publik = shareLink + /ssh
const DEFAULT_URL = 'https://ae0abd1c5b7346e80.sg.agentos-app.run/ssh';
...
// Mode stdio: untuk ProxyCommand ssh (stdin/stdout <-> ws)
function stdioMode(url) {
  const ws = dial(url);
  const pending = [];
  let opened = false;

  // Buffer stdin sampai ws 'open' — mencegah "Bad packet length"
...
// Mode listener: dengar TCP lokal, setiap koneksi buka ws baru
