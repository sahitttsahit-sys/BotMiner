...
// === diisi otomatis dari publish (step 4) ===
const DEFAULT_URL = 'https://a2854543c34f31fa9.sg.agentos-app.run/ssh';
...
// --stdio mode: untuk ProxyCommand (stdin/stdout <-> ws)
function stdioMode(url){
  const ws = dial(url);
  const pending=[]; let opened=false;
  // JANGAN resume stdin sebelum ws 'open' -> buffer dulu (hindari "Bad packet length")
...
// listener mode (default): dengar TCP lokal, tiap koneksi buka 1 ws
