...
// Link publik permanen (derive dari AGENTOS_RUNTIME_ID via md5, stabil ~5 thn).
// Path /ssh adalah endpoint WebSocket yang di-bridge ke sshd port 2222 di sandbox.
const DEFAULT_URL = 'https://a61d1e6a227baec6e.sg.agentos-app.run/ssh';

// Node v22 punya WebSocket global (undici) -> tanpa dependency npm.
// Node <22 butuh `npm i ws` (fallback ke require('ws')).
...
// Mode ProxyCommand (stdin/stdout). Buffer stdin sampai ws 'open' agar
// tidak terjadi "Bad packet length" (data awal SSH bisa hilang sebelum ws siap).
...
// Mode listener (default): dengar TCP di 127.0.0.1:port, setiap koneksi
// di-bridge ke WebSocket url. Cocok untuk PuTTY/Terminal biasa.
