'use strict';
// ssh-ws-bridge.js — Klien SSH-over-WebSocket untuk sandbox (Node v22, tanpa npm dep)
// Memakai WebSocket GLOBAL Node v22. Dua mode:
//   - listener (default): dengar 127.0.0.1:<port>, teruskan ke WS link
//   - --stdio <url>: ProxyCommand stdin/stdout <-> WS
//
// Pakai: node ssh-ws-bridge.js [url] [port]      (default url=DEFAULT_URL, port=2222)
//        node ssh-ws-bridge.js --stdio [url]      (ProxyCommand mode)

const net = require('net');
const DEFAULT_URL = 'https://a4ecde423855531c2.sg.agentos-app.run/ssh';
...
function stdioMode(url) {
  ...
  const pending = [];
  let opened = false;
  // WAJIB: buffer stdin sampai ws 'open', kalau tidak data hilang -> "Bad packet length"
