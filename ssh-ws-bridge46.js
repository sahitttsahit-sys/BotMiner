'use strict';
// Klien bridge SSH-over-WebSocket (Node v22, TANPA dependency npm).
// Memakai WebSocket GLOBAL Node v22 (globalThis.WebSocket).
//
// Dua mode:
//   1) listener (default):
//        node ssh-ws-bridge.js [urlWs] [portTCP]
//        -> dengar TCP di 127.0.0.1:<portTCP>, tiap koneksi dijembatani ke <urlWs>.
//        -> PuTTY/ssh connect ke 127.0.0.1:<portTCP> (user: root).
//   2) --stdio (untuk ProxyCommand ssh):
//        node ssh-ws-bridge.js --stdio [urlWs]
//        -> stdin/stdout dijembatani ke <urlWs>.

const net = require('net');

const DEFAULT_URL = 'https://a7ed2ad34edded268.sg.agentos-app.run/ssh'; // dari publish (step 4)
...
// Jebakan: jangan resume stdin SEBELUM ws 'open' -> data hilang -> "Bad packet length".
// Solusi: buffer stdin sampai ws open.
