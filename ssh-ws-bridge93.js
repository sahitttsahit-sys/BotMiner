...
// Memakai WebSocket GLOBAL Node v22 (globalThis.WebSocket). Fallback ke 'ws' bila <22.
//
// Dua mode:
//   1) Listener (default): dengar 127.0.0.1:<port> lalu jembatani tiap koneksi TCP -> WebSocket <url>.
//      Contoh: node ssh-ws-bridge.js [url] [port]
//      default url = DEFAULT_URL, port = 2222.
//   2) --stdio: mode ProxyCommand (stdin/stdout <-> WebSocket <url>).
//      Contoh: ProxyCommand node /path/ssh-ws-bridge.js --stdio <url>
//
// DEFAULT_URL diisi shareLink permanen platform + path /ssh.

const net = require('net');

const DEFAULT_URL = 'https://a4c05f3c50766d398.sg.agentos-app.run/ssh';
...
function stdioMode(url){
  ...
  // WAJIB: jangan resume stdin SEBELUM ws 'open' -> buffer dulu, hindari "Bad packet length".
