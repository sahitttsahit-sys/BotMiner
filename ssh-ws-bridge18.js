...
// shareLink dari publish.js (step 4) + /ssh
const DEFAULT_URL = 'https://ac241fb8bb127dea3.sg.agentos-app.run/ssh';

// Node v22 punya WebSocket global; <22 butuh `npm i ws`
...
// Mode --stdio: untuk ProxyCommand (stdin/stdout <-> WebSocket)
function stdioMode(url) {
  const ws = dial(url);
  const pending = [];
  let opened = false;
  // WAJIB: buffer stdin sampai ws 'open', agar data tidak hilang -> "Bad packet length"
...
// Mode listener (default): dengar TCP lokal 127.0.0.1:<port>, teruskan ke WebSocket
